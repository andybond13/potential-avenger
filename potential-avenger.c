...
cout << sbegin << " - " << send << endl;
        ...
        //print to vtk and other files
//        if (printVTK) {
//            printToFile(i,x,u,v,phi,d,s,e,Yc);   
//        }
    ...
    for (unsigned i = 0; i < nfrags[Ntim-1]/2; ++i) {
        ...
        sumfrag += fragLen;
        cout << "\afragment " << i << " length = " << fragLen << endl;
    }
...
/*
void PotentialAvenger::plotAtNodes ( const std::vector<unsigned>& NodalIds ) {
	if (_myid == 0) {
		...
		fprintf( pFileVr, "# GNUPLOT file generated by <PotentialAvenger.h>");
		...
		fprintf( pFileVt, "# GNUPLOT file generated by <PotentialAvenger.h>");
		...
		for ( unsigned i = 0; i < _NodesToPlot.size(); i++ ) {
		    ...
		    fprintf( pFile, "# GNUPLOT file generated by <PotentialAvenger.h>" );
...
void PotentialAvenger::plotAtElms ( const std::vector<unsigned>& ElmIds ) {
	if (_myid == 0) {
		...
		fprintf( pFileS, "# GNUPLOT file generated by <PotentialAvenger.h>");
...
/*
void PotentialAvenger::plotCohLaw (const std::vector<unsigned>& cohNums){
