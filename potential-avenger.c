...
void PotentialAvenger::printToFile(unsigned t, vector<double>& x, vector<double>& u, vector<double>& v, vector<double>& phi, vector<double>& d, vector<double>& s, vector<double>& e, double& Yc) {

    unsigned Nnod = x.size();
    unsigned Nelt = Nnod-1;
    
//    string vtkFile = "results"+std::to_string(t)+".vtk";
//    string vtkFile = "results.vtk";
    string vtkFile = "results"+convertInt(t)+".vtk";
    FILE * pFile;
    pFile = fopen( vtkFile.c_str(), "w" );
    fprintf ( pFile, "# vtk DataFile Version 2.0\n" );
    fprintf ( pFile, "Generated by <potential-avenger> -- AJS [DCML] (2013)\n" );
    fprintf ( pFile, "ASCII\nDATASET UNSTRUCTURED_GRID\n\n" );

	int var = 0;
    fprintf ( pFile, "POINTS %d double\n", Nnod);
    ...
    fprintf( pFile, "\nSCALARS strain double\n" );
    fprintf( pFile, "LOOKUP_TABLE default\n" );
    for ( unsigned i = 0; i < Nelt; i++ ) {
        fprintf( pFile, " %12.3e\n", e[i] );
    }
    fprintf( pFile, "\nSCALARS damage double\n" );
    fprintf( pFile, "LOOKUP_TABLE default\n" );
    for ( unsigned i = 0; i < Nelt; i++ ) {
        fprintf( pFile, " %12.3e\n", d[i] );
    }
